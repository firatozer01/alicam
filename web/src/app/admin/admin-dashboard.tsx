...
export function AdminDashboard() {
  ...
  return <main className="admin-page">
    <aside className="admin-sidebar"><Link className="brand admin-brand" href="/">alıcam<span>.net</span></Link><div className="admin-product"><span>YÖNETİM MERKEZİ</span><strong>Operasyon</strong></div><nav><Link className="active" href="/admin"><i>◇</i> Genel bakış</Link><Link href="/admin/satici-onaylari"><i>✓</i> Satıcı onayları <b>{data?.stats.pending_sellers || ""}</b></Link></nav><div className="admin-account"><span>{admin?.name.slice(0, 2).toLocaleUpperCase("tr-TR") ?? "AD"}</span><p><strong>{admin?.name ?? "Yönetici"}</strong><small>{admin?.email ?? "Oturum doğrulanıyor"}</small></p></div></aside>
